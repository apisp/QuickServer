...
import net.apisp.quick.old.server.RequestExecutorInfo;
...
public class HttpRequestResolver {
    ...
    public static class HttpRequestInfo implements StdHttpRequest {
        ...
        public StandardHttpCookie cookie(String key) {
            ...
            return new StandardHttpCookie(key, value);
        }

        @Override
        public StandardHttpCookie[] cookies() {
            StandardHttpCookie[] httpCookies = new StandardHttpCookie[cookies.size()];
            Iterator<Map.Entry<String, String>> cookieIterator = cookies.entrySet().iterator();
            int i = 0;
            Map.Entry<String, String> entry = null;
            while (cookieIterator.hasNext()) {
                entry = cookieIterator.next();
                httpCookies[i++] = new StandardHttpCookie(entry.getKey(), entry.getValue());
            }
            return httpCookies;
        }
        ...
        @Override
        public void setRequestExecutorInfo(RequestExecutorInfo requestExecutorInfo) {
            this.executorInfo = requestExecutorInfo;
        }

        @Override
        public void onExecuteOver() {
            if (this.getReqData() != null) {
                this.getReqData().close();
            }
        }
    }
}
